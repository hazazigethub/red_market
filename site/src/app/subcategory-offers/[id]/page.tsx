...
const features = [
  ...
  },
  {
    ...
    title: 'بلا عمولة',
    body: 'لا نأخذ نسبة من مبيعاتك. اشتراك ثابت فقط، والباقي لك.',
  },
];
...
function planItems(p: Plan): string[] {
  if (p.features && p.features.length > 0) return p.features;
  ...
  if (p.product_limit != null) {
    items.push(
      p.product_limit === -1
        ? 'عروض غير محدودة'
        : `حتى ${p.product_limit} عرض`
    );
  }
  if (p.reels_limit != null) {
    items.push(
      p.reels_limit === -1 ? 'ريلز غير محدودة' : `حتى ${p.reels_limit} ريلز`
    );
  }
  if (p.has_basic_reports) items.push('تقارير أساسية');
  if (p.has_detailed_reports) items.push('تقارير تفصيلية');
  return items;
}
  ...
  return (
    <main>
      ...
      <section id="plans" className="max-w-5xl mx-auto px-4 py-16">
        ...
        {plans.length === 0 ? (
          <div className="text-center border border-dashed border-gray-300 rounded-xl py-12 px-6">
            <p className="font-bold mb-2">
              لا توجد باقات {yearly ? 'سنوية' : 'شهرية'} متاحة حالياً
            </p>
            <p className="text-sm text-gray-600">
              {yearly
                ? 'جرّب عرض الباقات الشهرية، أو تواصل معنا لمعرفة الأسعار.'
                : 'تواصل معنا لمعرفة الأسعار المتاحة لمتجرك.'}
            </p>
          </div>
        ) : (
              ...
              return (
                <div
                  ...
                  className={`relative bg-white rounded-xl p-7 flex flex-col ${
                    isPro ? 'border-2 shadow-sm' : 'border border-gray-200'
                  }`}
        ...
        <p className="text-center text-xs text-gray-500 mt-8 leading-6">
          {yearly
            ? 'تسترد مبلغك كاملاً خلال 14 يوماً من الاشتراك السنوي'
      ...
      <section className="max-w-3xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-600 mb-8 leading-8">
          سجّل متجرك من لوحة التاجر، وسنساعدك في تجهيزه ورفع أول عروضك.
        </p>
